...
func NewSentryHook(levels []logrus.Level, options ...Option) SentryHook {
	h := SentryHook{
		levels:    levels,
...
func DefaultConverter(entry *logrus.Entry, event *sentry.Event, hub *sentry.Hub) {
	...
	if entry.Caller != nil {
		event.Tags["source"] = fmt.Sprintf("%s:%d", path.Base(entry.Caller.File), entry.Caller.Line)
	}
